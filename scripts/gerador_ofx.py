...
def gerar_arquivo_ofx(
    ...
    for transacao in transacoes_ordenadas:
        ...
        memo = limpar_texto_ofx(transacao.get('memo') or transacao.get('historico', 'Transação'))
        data = transacao['data']
        ...
        fitid = transacao.get('fitid') or gerar_fitid_por_dia(data, contadores_por_dia[data_chave])
