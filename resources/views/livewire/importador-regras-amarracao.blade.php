        ...
        <div class="bg-white overflow-hidden shadow-xl sm:rounded-lg">
            <div class="p-6 sm:p-8">
                <div class="flex items-start justify-between gap-4">
                    <div>
                        <h2 class="text-2xl font-bold text-gray-900">Importar Regras de Amarração</h2>
                        ...
                        @if($empresaAtual)
                            ...
                            @if($layoutNome)
                                <p class="mt-1 text-sm text-indigo-700">
                                    Layout: <span class="font-medium">{{ $layoutNome }}</span>
                                </p>
                            @endif
                        @else
                    ...
                    <a href="{{ $urlVoltarRegras }}" class="shrink-0 text-sm text-indigo-600 hover:text-indigo-800">← Voltar</a>
                </div>
                ...
                <div x-data="{ passo: @entangle('step') }">
                    {{-- Passo 1: arquivo --}}
                    <div wire:key="regras-passo-1" class="w-full" x-show="passo === 1"
                        ...
                        <x-zona-upload
                            input-id="arquivo-regras-amarracao"
                    ...
                    <div wire:key="regras-passo-2" class="w-full" x-show="passo === 2" x-cloak
                         x-transition:enter="transition ease-out duration-300"
                         x-transition:enter-start="opacity-0 translate-x-8"
                         x-transition:enter-end="opacity-100 translate-x-0">

                        <h3 class="text-xl font-semibold text-gray-900">2. Configure a importação</h3>
                        <p class="mt-1 mb-5 text-sm text-gray-500">Como as regras devem ser gravadas</p>
                        ...
                        <div class="space-y-4">
                            ...
                            <div>
                                ...
                                <select wire:model="layout_padrao" class="w-full border-gray-300 rounded-xl shadow-sm focus:ring-indigo-500 focus:border-indigo-500 py-3.5 px-4 text-base">
                                ...
                                <p class="mt-1 text-xs text-gray-500">Deixe em branco se o arquivo já traz o layout em uma coluna.</p>
                            </div>
                        ...
                        @if($estrategia === 'substituir_layout')
                            <p class="mt-4 text-sm text-amber-800 bg-amber-50 border border-amber-200 rounded-lg px-3 py-2">
                                As regras dos layouts presentes no arquivo serão removidas antes da importação.
                            </p>
                        @endif
                    ...
                    {{-- Passo 3: mapeamento --}}
                    <div wire:key="regras-passo-3" class="w-full" x-show="passo === 3" x-cloak
                         x-transition:enter="transition ease-out duration-300"
                         x-transition:enter-start="opacity-0 translate-x-8"
                         x-transition:enter-end="opacity-100 translate-x-0">

                        <h3 class="text-xl font-semibold text-gray-900">3. Mapeie as colunas</h3>
                        <p class="mt-1 mb-5 text-sm text-gray-500">Relacione as colunas do arquivo com os campos das regras</p>
                                ...
                                @foreach($campos as $campo => $label)
                                    <div>
                                        <label class="block text-sm font-medium text-gray-700 mb-1.5">
                                            {{ $label }}
                                            @if($campo === 'palavra_chave' || ($campo === 'layout' && !$layout_padrao))
                    ...
                    @if($step === 4 && !empty($preview))
                        <div wire:key="regras-passo-4" class="w-full"
                             x-transition:enter="transition ease-out duration-300"
                             x-transition:enter-start="opacity-0 translate-x-8"
                             x-transition:enter-end="opacity-100 translate-x-0">
                            ...
                            <dl class="mb-5 rounded-xl bg-gray-50 border border-gray-200 divide-y divide-gray-200 text-sm">
                                ...
                                <div class="flex justify-between gap-4 px-4 py-3">
                                    ...
                                    <dd class="font-medium text-green-700">{{ $preview['regras_novas'] ?? 0 }}</dd>
                                </div>
                                <div class="flex justify-between gap-4 px-4 py-3">
                                    <dt class="text-gray-500 shrink-0">Atualizadas</dt>
                                    <dd class="font-medium text-indigo-700">{{ $preview['regras_atualizadas'] ?? 0 }}</dd>
                                </div>
                                <div class="flex justify-between gap-4 px-4 py-3">
                            ...
                            @if(($preview['regras_ignoradas'] ?? 0) > 0)
                                <p class="text-sm text-gray-600 mb-3">{{ $preview['regras_ignoradas'] }} regra(s) ignorada(s) (já existentes).</p>
                            @endif
                            @if(($preview['regras_removidas'] ?? 0) > 0)
                                <p class="text-sm text-amber-800 bg-amber-50 border border-amber-200 rounded-lg px-3 py-2 mb-3">
                                    {{ $preview['regras_removidas'] }} regra(s) serão removidas.
                                </p>
                            @endif
                            @if(($preview['linhas_aviso'] ?? 0) > 0)
                                <p class="text-sm text-amber-800 bg-amber-50 border border-amber-200 rounded-lg px-3 py-2 mb-3">
                                    {{ $preview['linhas_aviso'] }} aviso(s) de conta não encontrada no plano de contas.
                                </p>
                            @endif
                            ...
                            @if(!empty($preview['avisos']))
                                <div class="mb-5 max-h-40 overflow-y-auto bg-amber-50 border border-amber-200 rounded-xl p-3 text-sm">
                                    @foreach(array_slice($preview['avisos'], 0, 30) as $aviso)
                                        <div class="text-amber-800">Linha {{ $aviso['linha'] }}: {{ $aviso['mensagem'] }}</div>
                                    @endforeach
                                </div>
                            @endif
                                ...
                                <div class="overflow-x-auto mb-5 rounded-xl border border-gray-200">
                                    <table class="min-w-full text-sm divide-y divide-gray-200">
                                        <thead class="bg-gray-50">
                                            <tr>
                                                <th class="px-4 py-2 text-left text-gray-500 font-medium">Layout</th>
                                                <th class="px-4 py-2 text-left text-gray-500 font-medium">Palavra-chave</th>
                                                <th class="px-4 py-2 text-left text-gray-500 font-medium">Conta</th>
                                        ...
                                        <tbody class="divide-y divide-gray-100">
                                            @foreach($preview['amostra'] as $item)
                                                <tr>
                                                    <td class="px-4 py-2 text-gray-700">{{ $item['layout_avancado'] }}</td>
                                                    <td class="px-4 py-2 text-gray-700">{{ $item['palavra_chave'] }}</td>
                                                    <td class="px-4 py-2 font-mono text-gray-900">{{ $item['conta_contrapartida'] ?: '—' }}</td>
                                                    <td class="px-4 py-2 text-gray-600">{{ ($item['_existe'] ?? false) ? 'Atualizar' : 'Incluir' }}</td>
                                                </tr>
                                            @endforeach
                                        </tbody>
                                    </table>
                                    <p class="text-xs text-gray-500 px-4 py-2 bg-gray-50">Amostra das primeiras 20 regras válidas.</p>
                                </div>
                            @endif
                            ...
                            <div class="flex flex-col sm:flex-row gap-4">
                                ...
                                <button type="button" wire:click="confirmarImportacao" wire:loading.attr="disabled"
                                        @disabled($processando || (empty($preview['regras_validas']) && $estrategia !== 'validar_apenas'))
