...
<aside
    class="group/sidebar fixed inset-y-0 left-0 z-40 flex h-screen w-24 flex-col border-r border-gray-700 bg-gray-900 text-white shadow-xl transition-transform duration-300 ease-in-out lg:translate-x-0 lg:transition-none"
            ...
            <div
                x-data="{
                    ...
                    abrir() {
                        ...
                        this.posicionar();
                        this.open = true;
                        this.$dispatch('submenu-nav-abrir', { id: {{ $menuIndex }} });
                    },
                    agendarFechar() {
                        clearTimeout(this.closeTimer);
                ...
                @mouseenter="abrir()"
                @mouseleave="agendarFechar()"
                @submenu-nav-abrir.window="if ($event.detail.id !== {{ $menuIndex }}) fechar()"
                @keydown.escape.window="fechar()"
                @scroll.window="if (open) fechar()"
            >
                <button type="button"
                        x-ref="trigger"
                ...
                <template x-teleport="body">
                    <div x-show="open"
                         ...
                         @mouseenter="abrir()"
                         @mouseleave="agendarFechar()"
                         @click.outside="if (!($refs.trigger && $refs.trigger.contains($event.target))) fechar()"
                         class="fixed z-[60] min-w-44 rounded-lg border border-gray-700 bg-gray-900 py-1 shadow-xl"
                         style="background-color: #111827;"
                        ...
                        @foreach($menu['items'] as $item)
                            @if($item['disabled'] ?? false)
                                <span class="flex cursor-not-allowed items-center gap-2.5 px-3 py-2 text-sm text-gray-500"
                            ...
                            @else
                                <a href="{{ $item['url'] }}"
                                   ...
                                   class="flex items-center gap-2.5 px-3 py-2 text-sm transition-colors {{ ($item['active'] ?? false) ? 'bg-indigo-600 text-white' : 'text-gray-300 hover:bg-gray-800 hover:text-white' }}"
                                   @click="fechar(); fecharMenu()">
